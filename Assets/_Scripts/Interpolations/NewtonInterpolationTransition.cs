...
public class NewtonInterpolationTransition : BaseInterpolationScript
{
    ...
    private Quaternion NewtonInterpolation(float currentTransitionT, List<InterpolationNode> nodes)
    {
        ...
        for (var i = 1; i < nodes.Count; i++)
            for (var j = 0; j < i; j++)
                a[i] = (a[i] - a[j]) / (nodes[i].rotation.x - nodes[j].rotation.x);
        ...
        for (var i = 1; i < nodes.Count; i++)
            for (var j = 0; j < i; j++)
                a[i] = (a[i] - a[j]) / (nodes[i].rotation.y - nodes[j].rotation.y);
        ...
        for (var i = 1; i < nodes.Count; i++)
            for (var j = 0; j < i; j++)
                a[i] = (a[i] - a[j]) / (nodes[i].rotation.z - nodes[j].rotation.z);
        ...
        for (var i = 1; i < nodes.Count; i++)
            for (var j = 0; j < i; j++)
                a[i] = (a[i] - a[j]) / (nodes[i].rotation.w - nodes[j].rotation.w);
